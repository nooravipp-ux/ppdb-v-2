...
// add testimoni
$(document).on("submit", "#modalForAddTestimoni", $(this).serialize(), function(e) {
    e.preventDefault();
    $.ajaxSetup({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    // var formData = $(this).serialize()
console.log($('#nama_alumni').val());
    var nama_alumni = $('#nama_alumni').val();
    var lulusan = $('#lulusan').val();
    var testimoni = $('#testimoni').val();
    var status_testimoni = $('#status_testimoni').val();
    $.ajax({
        type: "post",
